...
export const saveInterestedEvent = (payload) => {
    return async (dispatch, getState) => { 
        try {
            dispatch(showLoaderAction())
            const url = `https://node-final-project-j4jc.onrender.com:9000/saveEvent`
...
export const getAllSavedEvents = (userUUID) => {
    return async (dispatch, getState) => { 
        try {
            const url = `https://node-final-project-j4jc.onrender.com:9000/getAllSavedEvents/${userUUID}`
            const response = await HTTP.get(url);
...
export const unsubscribeEvent = (userUUID, eventId) => {
    return async (dispatch, getState) => { 
        try {
            const url = `https://node-final-project-j4jc.onrender.com:9000/getAllSavedEvents/${userUUID}/${eventId}`
